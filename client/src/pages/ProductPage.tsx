...
const ProductPage = () => {
  ...
  // fetch product data using this id .when component loaded useeffect runs automatically
  const { id } = useParams()
  const navigate = useNavigate()
  const { items, addToCart, updateQuantity, removeFromCart } = useCart()

  const [product, setProduct] = useState<Product | null>(null)
  // here we are stroing multiple produts that why we need product array
  const [relatedProducts, setRelatedProducts] = useState<Product[]>([])
  const [loading, setLoading] = useState(true)
  // required items for 1 product like apple->ineed 2 quantity
  const [localQuantity, setLocalQuantity] = useState(1)

  // it executes each every time after we clicking on it if we click on 2 id it navigate to that place 
  ...
  useEffect(() => {
    ...
    // ADDING API AND REMOVING DUMMY DATA
        ...
        <div className="bg-white/50 rounded-2xl overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="relative flex items-center justify-center p-8 md:p-12 min-h-[320px] md:min-h-[480px]">
              ...
              {/* Badgs means offer showing product top */}
