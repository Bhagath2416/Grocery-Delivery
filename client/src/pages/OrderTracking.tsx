...
const OrderTracking = () => {
  // to find individual id
  const currency=import.meta.env.VITE_CURRENCY_SYMBOL || "$";
  const { id } = useParams();
  const navigate = useNavigate()
  const [order, setOrder] = useState<Order | null>(null)
  const [loading, setLoading] = useState(true)
  // this object contains two properties one longitute and latitude
  ...
  return (
    // if order details have go to div
    <div className="min-h-screen mb-20 bg-app-cream">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* live traction on left and product detail on right */}
        <div className="grid lg:grid-cols-3 gap-6">
          {/* left side - Timeline + Map Area */}
          <div className="lg:col-span-2 space-y-6">

            {/*  OTP CARD -> directly imported*/}
