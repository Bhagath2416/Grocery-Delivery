...
// WHEN THE REDUCED TO 10 THEN SEND THE EMAIL TO ADMIN FOR EVERY PRODUCT
...
const checkLowStock = inngest.createFunction(
    // add object and function
    // this is object
    { id: "check-low-stock", 
        name: "Low Stock Alert",
        triggers: [{ event: "inventory/stock.updated" }] },
        // this is function
        // the async excutes when it trigger events
    async ({ event, step }) => {
...
// if there is no email then return message
        if(adminEmails.length === 0)  return {skipped: true, reason: "No admin emails"}
// Now send email to each admin use the nodemailer Search->Nodemailer.com and install typescript types definitions
// in this nodemailer docs having transporter add this code
// transporter is used to send email
          
        await sendEmail ({
            // receiver email id,send multiple emails
            to:adminEmails.join(","),
...
//  Monthly Offers Email(1st of every month - payday),it will be a cron job
...
const sendMonthlyOffers = inngest.createFunction({
  ...
  let sentCount=0;
//   Send in batches of 10 to avoid overwhelming mail sever
...
const autoAssignRider = inngest.createFunction({
 ...
 name: "Auto Assign Deliver Rider",
 triggers: [{event: "order/placed"}]
}, async ({event,step})=>{
    // we get order id from event.data
...
const result= await step.run("assign-rider", async ()=> {
    ...
    const busyRiderIds=busyOrders.map((o)=>order.deliveryPartnerId)
...
// 1.send low stock to admin
//  2.send every month on 1st date offer to emaol to the user
// 3.auto assign rider for the order