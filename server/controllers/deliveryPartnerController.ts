...
return res.status(400).json({message: "Please provide email and password"})
 ...
 if(!partner.isActive){
    return res.status(403).json({message: "Your accounthas been deactivated"})
 }

//  partner.password stored in db
...
res.json({order: updatedOrder, message: "Delivery completed successsfully"})
}
...
//  after finding order data update the lat,lng
